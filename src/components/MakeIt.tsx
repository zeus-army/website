...
const MarketCapSelector = styled.div`
  display: flex;
  justify-content: center;
  align-items: center;
  gap: 1rem;
...
const MarketCapChip = styled.button<{ $selected: boolean }>`
  ...
  border-radius: 25px;
  border: 2px solid ${props => props.$selected ? 'var(--color-primary)' : 'rgba(255, 215, 0, 0.3)'};
  background: ${props => props.$selected
    ? 'linear-gradient(135deg, #FFD700 0%, #FFA500 100%)'
    : 'rgba(10, 14, 39, 0.8)'};
  color: ${props => props.$selected ? '#000' : 'var(--color-text-light)'};
  font-family: var(--font-body);
  font-size: 1.1rem;
  font-weight: 700;
  cursor: pointer;
...
const MakeIt: React.FC = () => {
  ...
  // Calculate projected USD value based on target market cap
  const calculateProjectedValue = (holder: Holder): string => {
    if (marketCap === 0 || price === 0) return '$0.00';

    // Calculate target price based on market cap ratio
    const targetPrice = price * (targetMarketCap / marketCap);

    // Calculate projected value
    const projectedValue = holder.totalBalanceRaw * targetPrice;

    return formatUSD(projectedValue.toFixed(2));
  };
  ...
  return (
    <HoldersSection>
      <Container>
        ...
        <MarketCapSelector>
          <MarketCapLabel>Target Market Cap:</MarketCapLabel>
          <MarketCapChip
            $selected={targetMarketCap === 500_000_000}
            onClick={() => setTargetMarketCap(500_000_000)}
          >
            $500M
          </MarketCapChip>
          <MarketCapChip
            $selected={targetMarketCap === 1_000_000_000}
                ...
                return (
                  <React.Fragment key={holder.address}>
                    <TableRow isTop3={holder.rank <= 3}>
                      ...
                      <TableCell>
                        <USDValue>{calculateProjectedValue(holder)}</USDValue>
                      </TableCell>
                    </TableRow>
                      ...
                      <>
                        {poolLPHolders.map((lpHolder, idx) => (
                          <SubRow key={`${holder.address}-lp-${idx}`}>
                            ...
                            <TableCell>
                              <USDValue>{calculateProjectedValue(lpHolder)}</USDValue>
                            </TableCell>
                          </SubRow>
                        ))}
                      ...
                      <>
                        {poolWZEUSHolders.map((wzeusHolder, idx) => (
                          <SubRow key={`${holder.address}-wzeus-${idx}`}>
                            ...
                            <TableCell>
                              <USDValue>{calculateProjectedValue(wzeusHolder)}</USDValue>
                            </TableCell>
                          </SubRow>
                        ))}
